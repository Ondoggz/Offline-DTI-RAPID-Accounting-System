// client/src/pages/ReportModule.jsx
...
const ReportModule = () => {
  ...
  const generateReport = async () => {
    ...
    try {
      let endpoint = 'http://localhost:5000/api/reports/';
      let body = { reportType };
  ...
  const renderSalesChart = () => {
    ...
    if (rangeType === 'single' && reportData.data.organization) {
      ...
      // Bar chart for sales and volume
      const barData = {
      ...
      // Pie chart for bean type distribution
      let pieData = null;
          ...
          datasets: [
            {
              ...
              data: Object.values(org.beanTypeSummary).map(v => v.salesGenerated),
    ...
    } else if (rangeType === 'range' && reportData.data) {
      const lineData = {
        labels: reportData.data.map(d => d.monthName),
        datasets: [
          {
            label: 'Volume Sold (kg)',
            data: reportData.data.map(d => d.organization.totalVolumeSold || 0),
            borderColor: '#4CAF50',
          ...
          },
          {
            ...
            data: reportData.data.map(d => d.organization.totalSalesGenerated || 0),
            borderColor: '#FF9800',
        ...
        <div className="report-content" id="report-content">
          ...
          {reportType !== 'per-farmer' && (
            <div className="organization-summary">
              ...
              {/* Bean Type Breakdown */}
              {rangeType === 'single' && reportData.data.organization?.beanTypeSummary && 
               Object.keys(reportData.data.organization.beanTypeSummary).length > 0 && (
                <div className="bean-breakdown">
                  ...
                  <table className="report-table">
                    ...
                    <tbody>
                      {Object.entries(reportData.data.organization.beanTypeSummary).map(([beanType, data]) => (
                        <tr key={beanType}>
          ...
          {/* Monthly Breakdown for Range Report */}
          {rangeType === 'range' && reportData.data && (
          ...
          {/* Per-Farmer Report */}
          {reportType !== 'organization' && reportData.data.perFarmer && reportData.data.perFarmer.length > 0 && (
                ...
                <table className="report-table">
                  ...
                  <tfoot>
                    <tr className="total-row">
                      ...
                      <td><strong>{reportData.data.perFarmer.reduce((sum, f) => sum + (f.volumeSold || 0), 0).toFixed(2)}</strong></td>
                      <td><strong>{formatCurrency(reportData.data.perFarmer.reduce((sum, f) => sum + (f.salesGenerated || 0), 0))}</strong></td>
                    </tr>
                  </tfoot>
                </table>
              </div>
